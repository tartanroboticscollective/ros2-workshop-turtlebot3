...
linux_post_install() {
    ...
    # The current shell does not automatically pick up the new group membership.
    # Test Docker using a new shell with the docker group.
    if sg docker -c "docker run hello-world"; then
        echo "Running Docker without sudo now!"
    else
        echo "Docker test failed. You may need to log out and back in first."
    fi
